...
async function init() {
  try {
    ...
    const saved = fin.savings?.totalSavings ?? fin.savings?.current ?? 0;
    ...
    setText('tile-target-sub', fin.goal?.movingWindow || '');
  ...
  try {
    ...
    if (ul) {
      ...
      ul.innerHTML = items.length
        ? items.map((a) => `<li><strong>${esc(a.name)}</strong> <span class="muted">· ${esc(a.town)}</span></li>`).join('')
